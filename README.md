...
###### 注意：
1. ChangeHosts.py脚本设定的是运行完毕之后等待3秒自动结束（time.sleep(3)），可以在源码中取消等待（取消time.sleep(3)和import time）。
2. 在运行程序前，您务必将hosts文件内容的文本替换为“hosts_样例”中的文本，如果您不替换，就需要自行修改代码配适，否则会造成hosts文本内容紊乱
## 三、效果对比
之前的样子：
